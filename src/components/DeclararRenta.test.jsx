...
describe('DeclararRenta', () => {
  it('no muestra resultado hasta que se ingrese al menos un valor', () => {
    ...
    expect(screen.queryByText(/quedas obligado a declarar renta/i)).not.toBeInTheDocument();
    expect(screen.queryByText(/no quedas obligado a declarar renta/i)).not.toBeInTheDocument();
  });
  ...
  it('avisa que no queda obligado cuando ningún valor supera su tope', async () => {
    ...
    expect(await screen.findByText(/no quedas obligado a declarar renta/i)).toBeInTheDocument();
  });
  ...
  it('avisa que queda obligado y nombra el criterio superado (patrimonio bruto, 4.500 UVT)', async () => {
    ...
    expect(await screen.findByText(/quedas obligado a declarar renta/i)).toBeInTheDocument();
